...
The `AtlantisCmd`_ package provides the :command:`atlantis` command-line tool for doing various operations associated with the Salish Sea Atlantis project version of the CSIRO Atlantis ecosystem model.

`AtlantisCmd`_ is an extensible tool built on the OpenStack :kbd:`cliff`
(`Command Line Interface Formulation Framework`_)
package.
It is a command processor tool specific to the Salish Sea CSIRO Atlantis model that uses plug-ins from the `NEMO-Cmd`_ package.
